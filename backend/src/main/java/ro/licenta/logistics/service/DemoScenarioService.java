...
public class DemoScenarioService {
    ...
    public synchronized void reset() {
        version++;
        hubs.clear(); hubById.clear(); drivers.clear(); vehicles.clear(); shipments.clear();
    ...
    public synchronized List<DepotDto> depots() { return List.copyOf(hubs); }
    public synchronized DepotDto depot() { return hubById.get("HUB-B"); }
    public synchronized DepotDto hub(String id) { return hubById.get(id); }
    public synchronized Map<String, DepotDto> hubMap() { return new LinkedHashMap<>(hubById); }
    ...
    public synchronized List<VehicleDto> vans() { return vehicles.stream().filter(v -> "VAN".equals(v.kind())).toList(); }
    public synchronized List<VehicleDto> lineHaulVehicles() { return vehicles.stream().filter(v -> "LINEHAUL".equals(v.kind())).toList(); }
    public synchronized List<ShipmentDto> shipments() { return List.copyOf(shipments); }
    ...
    public synchronized void deleteShipment(long id) { version++; shipments.removeIf(s -> s.id() == id); }
    ...
    public synchronized void deleteVehicle(String code) { version++; vehicles.removeIf(v -> v.code().equals(code)); }
    ...
    private String codeFor(long id) { return "LO" + (1000000 + id); }
    private String timeWindow(int start, int end) { return minuteToTime(start) + "-" + minuteToTime(end); }
    private String minuteToTime(int m) { return String.format("%02d:%02d", m / 60, m % 60); }
    ...
    private VehicleDto van(String code, String driverId, String hubId) { return new VehicleDto(code, driverId, driverName(driverId), hubId, "VAN", VAN_CAPACITY_KG, VAN_CONSUMPTION, VAN_COST_RON_PER_KM); }
    private VehicleDto linehaul(String code, String driverId, String hubId) { return new VehicleDto(code, driverId, driverName(driverId), hubId, "LINEHAUL", LINEHAUL_CAPACITY_KG, LINEHAUL_CONSUMPTION, LINEHAUL_COST_RON_PER_KM); }
    private String driverName(String driverId) { return drivers.stream().filter(d -> d.id().equals(driverId)).map(DriverDto::name).findFirst().orElse("Șofer neasignat"); }
